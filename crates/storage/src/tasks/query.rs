...
impl Query {
    ...
    pub fn statuses(mut self, value: impl IntoIterator<Item = types::tasks::TaskStatus>) -> Self {
    ...
    pub async fn exec(&self, pool: &sqlx::PgPool) -> error::Result<QueryResult<types::tasks::Task>> {
        self.validate()?;
        ...
        let rows = qb
            .build_query_scalar::<sqlx::types::Json<types::tasks::Task>>()
            .fetch_all(pool)
            .await?
            .into_iter()
