...
impl Query {
    ...
    pub async fn exec(&self, pool: &sqlx::PgPool) -> error::Result<QueryResult<types::chats::Message>> {
        self.validate()?;
        ...
        let rows = qb
            .build_query_scalar::<sqlx::types::Json<types::chats::Message>>()
            .fetch_all(pool)
            .await?
            .into_iter()
        ...
        Ok(crate::paginate(rows, self.limit, |message| message.id))
    }
}
