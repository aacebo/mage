...
pub fn run(model: &mut Bart, config: &Config, input: &Tensor, device: &Device) -> Result<Vec<u32>> {
    ...
    let encoder_xs = model.encode(input).map_err(mage_error::ai)?;
    let (_, seq, hidden) = encoder_xs.dims3().map_err(mage_error::ai)?;
    let encoder_xs = encoder_xs
        .expand((beams, seq, hidden))
        .and_then(|xs| xs.contiguous())
        .map_err(mage_error::ai)?;
    ...
    for past_kv_len in 0..config.max_length {
        let input = Tensor::from_vec(tokens.clone(), (beams, 1), device).map_err(mage_error::ai)?;
        ...
        let order = Tensor::from_vec(order, next.len(), device).map_err(mage_error::ai)?;
        model.reorder_kv_cache(&order).map_err(mage_error::ai)?;
