...
fn pool(hidden: &Tensor, mask: &Tensor) -> Result<Tensor> {
    ...
    let summed = hidden.broadcast_mul(&mask).and_then(|v| v.sum(1)).map_err(mage_error::ai)?;
    let counts = mask.sum(1).map_err(mage_error::ai)?;
    summed.broadcast_div(&counts).map_err(mage_error::ai)
}
...
fn normalize(v: &Tensor) -> Result<Tensor> {
    ...
    v.broadcast_div(&norm).map_err(mage_error::ai)
}
