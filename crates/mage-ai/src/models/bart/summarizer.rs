...
impl Summarizer {
    pub fn new(config: &Config, vars: VarBuilder) -> Result<Self> {
        Ok(Self {
            model: Mutex::new(Bart::new(config, vars).map_err(mage_error::ai)?),
...
impl Summarizer {
    fn one(&self, cx: &Context, text: &str, generation: &generate::Config) -> Result<String> {
        ...
        let input = Tensor::from_slice(ids, (1, ids.len()), cx.device()).map_err(mage_error::ai)?;
        let mut model = self
            .model
            .lock()
            .map_err(|_| mage_error::ai("summarization model lock poisoned".to_string()))?;
