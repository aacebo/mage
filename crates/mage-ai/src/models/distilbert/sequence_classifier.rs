...
impl SequenceClassifier {
    pub fn new(vars: VarBuilder, config: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            pre_classifier: candle_nn::linear(hidden, hidden, vars.pp("pre_classifier")).map_err(mage_error::ai)?,
            classifier: candle_nn::linear(hidden, LABELS, vars.pp("classifier")).map_err(mage_error::ai)?,
        })
    }

    pub fn forward(&self, ids: &Tensor, padding: &Tensor) -> Result<Vec<Vec<f32>>> {
        let hidden = self.distilbert.forward(ids, padding)?;
        let pooled = hidden.i((.., 0)).map_err(mage_error::ai)?;
        self.pre_classifier
            .forward(&pooled)
            .and_then(|v| v.relu())
