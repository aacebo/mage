...
pub struct SearchOptions {
    ...
    pub role: Option<mage_types::actors::Role>,
}
...
impl SearchOptions {
    ...
    pub fn with_role(mut self, role: mage_types::actors::Role) -> Self {
        self.role = Some(role);
        self
    }
    ...
    pub fn validate(&self) -> Result<()> {
        if self.limit == 0 {
            return Err(mage_error::bad_request("search limit must be greater than zero"));
        }

        if !self.min_similarity.is_finite() || !(-1.0..=1.0).contains(&self.min_similarity) {
            return Err(mage_error::bad_request(
...
pub fn prepare(embedding: Vec<f32>, options: SearchOptions) -> Result<(Vector, i64, f64)> {
    ...
    if embedding.len() != EMBEDDING_DIMENSIONS {
        return Err(mage_error::bad_request(format!(
    ...
    if embedding.iter().any(|value| !value.is_finite()) {
        return Err(mage_error::bad_request("search embedding components must be finite"));
    }

    if embedding.iter().all(|value| *value == 0.0) {
        return Err(mage_error::bad_request("search embedding cannot be a zero vector"));
    }
