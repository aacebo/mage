...
pub struct Query {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub levels: Option<Vec<mage_types::logs::Level>>,
...
impl Query {
    ...
    pub fn levels(mut self, value: impl IntoIterator<Item = mage_types::logs::Level>) -> Self {
    ...
    pub async fn exec(&self, pool: &sqlx::PgPool) -> mage_error::Result<QueryResult<mage_types::logs::Log>> {
        self.validate()?;
        ...
        let rows = qb
            .build_query_scalar::<sqlx::types::Json<mage_types::logs::Log>>()
            .fetch_all(pool)
            .await?
            .into_iter()
