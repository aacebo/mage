...
impl<'a> LogStorage<'a> {
    ...
    pub async fn get_by_id(&self, id: uuid::Uuid) -> Result<Option<mage_types::logs::Log>> {
        let query = format!("SELECT {} FROM logs WHERE id = $1", project::jsonb_build_object("logs"));
        let log = sqlx::query_scalar::<_, Json<mage_types::logs::Log>>(sqlx::AssertSqlSafe(query))
            .bind(id)
    ...
    pub async fn get(&self, query: Query) -> Result<QueryResult<mage_types::logs::Log>> {
        query.exec(self.pool).await
    }

    pub async fn create(&self, log: mage_types::logs::Log) -> Result<mage_types::logs::Log> {
        sqlx::query(
            r#"
            INSERT INTO logs (
