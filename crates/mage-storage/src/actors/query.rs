...
pub struct Query {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub roles: Option<Vec<mage_types::actors::Role>>,
...
impl Query {
    ...
    pub fn roles(mut self, value: impl IntoIterator<Item = mage_types::actors::Role>) -> Self {
    ...
    pub async fn exec(&self, pool: &sqlx::PgPool) -> mage_error::Result<QueryResult<mage_types::actors::Actor>> {
        self.validate()?;
        ...
        let rows = qb
            .build_query_scalar::<sqlx::types::Json<mage_types::actors::Actor>>()
            .fetch_all(pool)
            .await?
            .into_iter()
