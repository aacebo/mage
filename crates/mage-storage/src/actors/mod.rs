...
impl<'a> ActorStorage<'a> {
    ...
    pub async fn get_by_id(&self, id: uuid::Uuid) -> Result<Option<mage_types::actors::Actor>> {
        let query = format!(
            "SELECT {} FROM actors WHERE actors.id = $1",
            project::jsonb_build_object("actors")
        );
        let actor = sqlx::query_scalar::<_, Json<mage_types::actors::Actor>>(sqlx::AssertSqlSafe(query))
            .bind(id)
    ...
    pub async fn get(&self, query: Query) -> Result<QueryResult<mage_types::actors::Actor>> {
    ...
    pub async fn get_by_external_id(
        ...
        tenant_id: uuid::Uuid,
        external_id: String,
    ) -> Result<Option<mage_types::actors::Actor>> {
        let query = format!(
            r#"SELECT {}
            FROM actors
    ...
    pub async fn search(
    ...
    ) -> Result<Vec<SearchResult<mage_types::actors::Actor>>> {
        let role = options.role.map(mage_types::actors::Role::as_str);
        ...
        sqlx::query("SET LOCAL hnsw.iterative_scan = strict_order")
            .execute(&mut *tx)
            .await?;
        let rows = sqlx::query_as::<_, (Json<mage_types::actors::Actor>, f64)>(sqlx::AssertSqlSafe(query))
            .bind(tenant_id)
            .bind(embedding)
            .bind(limit)
            .bind(min_similarity)
    ...
    pub async fn create(&self, actor: mage_types::actors::Actor) -> Result<mage_types::actors::Actor> {
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn update(&self, actor: mage_types::actors::Actor) -> Result<mage_types::actors::Actor> {
        let mut tx = self.pool.begin().await?;
        let embedding = actor.embedding.clone().map(Vector::from);
    ...
    pub async fn update_embedding(&self, id: uuid::Uuid, embedding: Vec<f32>) -> Result<mage_types::actors::Actor> {
        let result = sqlx::query(
            r#"
            UPDATE actors
            SET embedding = $2,
    ...
    pub async fn update_secret(&self, id: uuid::Uuid, value: impl std::fmt::Display) -> Result<mage_types::actors::Actor> {
        let result = sqlx::query(
            r#"
            UPDATE agents
            SET secret = $2,
    ...
    pub async fn connect(&self, id: uuid::Uuid) -> Result<Option<mage_types::actors::Actor>> {
        let mut tx = self.pool.begin().await?;
        let result = sqlx::query(
            r#"
            UPDATE agents
    ...
    pub async fn disconnect(&self, id: uuid::Uuid) -> Result<Option<mage_types::actors::Actor>> {
        let mut tx = self.pool.begin().await?;
        let result = sqlx::query(
            r#"
            UPDATE agents
